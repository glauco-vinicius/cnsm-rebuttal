...
**Comment 1 (V-05 contradiction).** Confirmed: HFCCT's `check_re_addr()` genuinely emits V-05 warnings, though this detector is undocumented in the tool's original 15-category specification (Li et al., Table 2). We verified this against the tool's public repository, a single unmodified commit predating the tool's own publication, ruling out a version difference. We also found the three chaincode names in the Table V footnote were incorrect due to a transcription error; the correct chaincodes are cp-digibank, cp-magneto, and token-utxo. Finding counts and Precision figures are unaffected. Table III now marks this detector as undocumented rather than not implemented, and Section IV-B is revised accordingly.

**Comment 2 (pre-filter agent).** The automated pre-filter is [MODEL NAME], prompted to classify each finding for verdict correctness, taxonomy category, and evidentiary support. We agree this should not be left implicit and have added the specification to Section III-E, noting the GLM-5.2 divergence (~45% automated versus ~51% manual estimate) as a useful calibration check, now discussed in Section IV-A.
